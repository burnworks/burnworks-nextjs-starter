...
3. `npm test`
4. `npm run build`
5. `NEXT_PUBLIC_SITE_URL` が本番 URL になっているか確認
6. `NEXT_PUBLIC_SITE_LAST_MODIFIED` をコンテンツの最終更新日に合わせて更新
7. カメラ、マイク、位置情報を使う場合は `next.config.ts` の `Permissions-Policy` を調整
8. 外部サービスとデプロイ構成に合わせて CSP・HSTS の追加を検討
